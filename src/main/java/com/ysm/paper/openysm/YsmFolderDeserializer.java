...
public class YsmFolderDeserializer implements AutoCloseable {
    ...
    private static ImageMeta parseImageMeta(byte[] data, String path) {
        ...
        try {
            ...
            switch (format) {
                case 1, 3 -> img = ImageIO.read(new ByteArrayInputStream(data)); // BMP/JPEG
                case 4, 5 -> throw new RuntimeException(
                        "WEBP/AVIF image dimensions are not supported by the PaperYSM Java legacy importer");
            }
            if (img != null) {
